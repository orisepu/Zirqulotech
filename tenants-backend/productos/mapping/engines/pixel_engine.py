"""
Google Pixel Mapping Engine - Orquestación completa del proceso de mapeo.

Coordina todos los componentes para mapear dispositivos Pixel:
1. Extractor: Parsea el texto crudo y extrae features
2. Knowledge Base: Enriquece con información (año, Tensor chip)
...
from productos.mapping.extractors.pixel_extractor import PixelFeatureExtractor
from productos.mapping.knowledge.pixel_kb import PixelKnowledgeBase
...
class PixelEngine(IMappingEngine):
    """
    Engine de mapeo para Google Pixel.
    ...
    def __init__(self):
        ...
        self.extractor = PixelFeatureExtractor()
        self.knowledge_base = PixelKnowledgeBase()

        # Matchers (en orden de prioridad)
        # Nota: Pixel no usa A-number, usa códigos de modelo (G9S9B, etc.)
        # pero esos códigos no están en la BD, solo en Likewize
        ...
        # Patrón para detectar Pixel
        self.pixel_pattern = re.compile(r'\bpixel\b', re.I)
    ...
    def can_handle(self, input_data: LikewizeInput) -> bool:
        ...
        Returns:
            True si es un Pixel
        """
        return bool(self.pixel_pattern.search(input_data.model_name))
    ...
    def map(self, input_data: LikewizeInput) -> MatchResult:
        ...
        context.info("=== Iniciando proceso de mapeo de Google Pixel ===")
        context.start_timer()
        ...
        try:
            ...
            # 4. Buscar candidatos con matcher (prioridad: Name → Generation)
            ...
            # Si no hay candidatos, usar GenerationMatcher como fallback
            if not candidates:
